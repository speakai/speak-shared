 ...
 * and the voice-agent backend. USD per 1M tokens. Verified 2026-06 against:
 *   OpenAI    — https://platform.openai.com/docs/pricing
 *   Anthropic — https://docs.anthropic.com/en/docs/about-claude/pricing  (cache read = 0.1x input)
 *   Gemini    — https://ai.google.dev/gemini-api/docs/pricing
 *
 * `cachedInputPerMillion` = cached-input read rate (omitted where the provider
 * doesn't publish one). `*Long` = long-context tier (per-request above
 * `longContextThresholdTokens`). Within each provider: DEPRECATED first, then LIVE.
 */
